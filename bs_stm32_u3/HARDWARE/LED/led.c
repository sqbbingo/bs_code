...
void LED_Init(void)
{
 ...
 GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6;				 //LED0-->PB. 端口配置
 ...
 GPIO_InitStructure.GPIO_Pin = GPIO_Pin_8;	    		 //LED2-->PB.8 端口配置, 推挽输出
 GPIO_Init(GPIOB, &GPIO_InitStructure);	  				 //推挽输出 ，IO口速度为50MHz
 GPIO_SetBits(GPIOB,GPIO_Pin_8); 						 //PB.8 输出高 

 GPIO_InitStructure.GPIO_Pin = GPIO_Pin_9;	    		 //LED3-->PB.9 端口配置, 推挽输出
 GPIO_Init(GPIOB, &GPIO_InitStructure);	  				 //推挽输出 ，IO口速度为50MHz
